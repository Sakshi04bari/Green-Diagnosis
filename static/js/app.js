...
async function handlePredict() {
  ...
  try {
    ...
    if (!response || !response.ok) {
      throw new Error(
        `Backend connection failed. Please ensure your Flask server is running on port 5000.\n\nSteps to fix:\n1. Run: python main.py\n2. Check that the server starts on http://localhost:5000\n3. Verify your model file exists\n\nLast error: ${lastError?.message || "Unknown error"}`,
      )
    }
    ...
    detectionHistory = detectionHistory.slice(0, 50) // Keep last 50 results
...
function updateHistoryStats() {
  ...
  const healthyCount = detectionHistory.filter((item) => item.result.disease.toLowerCase() === "healthy").length

  const sigatokaCount = detectionHistory.filter((item) => item.result.disease.toLowerCase() === "sigatoka").length

  const xanthomonasCount = detectionHistory.filter((item) => item.result.disease.toLowerCase() === "xanthomonas").length

  const avgConfidence =
    detectionHistory.length > 0
      ? Math.round(
          detectionHistory.reduce((acc, item) => acc + Number.parseFloat(item.result.confidence.replace("%", "")), 0) /
